...
export default function DashboardPortal() {
  ...
  const [actionType, setActionType] = useState<"approve" | "reject" | "process" | "processor-reject" | "view" | "edit" | "delete">("view");
  ...
  useEffect(() => {
    ...
    if (user?.role === "PROCESSOR") setActiveTab("processor");
    else if (user?.role === "ADMIN") setActiveTab("tracker");
    else setActiveTab("approver");
  }, []);
      ...
      {stats && (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-3 sm:gap-4 mb-6 sm:mb-8">
          <StatCard
            ...
            subtext={`${stats.totalRequests} total submissions`}
            emoji="💵"
          />
          <StatCard
            title="Total Paid (Disbursed)"
            value={`$${stats.totalProcessedAmount.toFixed(2)}`}
            subtext={`${stats.processed} approved & processed`}
            emoji="💳"
            valueColor="text-emerald-400"
          />
          <StatCard
            title="Awaiting Manager (User 1)"
          ...
          />
          <StatCard
            title="Awaiting Disbursal (User 2)"
            value={String(stats.pendingProcessing)}
            subtext="Approved, ready to pay"
            emoji="💸"
