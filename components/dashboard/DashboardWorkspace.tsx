...
export default function DashboardWorkspace() {
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute("download", `AceolutionFinance_Report_${new Date().toISOString().split("T")[0]}.csv`);
    ...
    <div className="flex min-h-0 w-full flex-1">
            ...
            <div className="flex shrink-0 items-center gap-2">
              {activeSection === "user-management" && currentUser?.role === "ADMIN" && (
                <button
                  ...
                  className="hidden sm:inline-flex px-4 py-2 rounded-lg bg-[var(--af-navy)] hover:bg-[var(--af-navy-soft)] text-xs font-bold text-white transition-colors cursor-pointer"
                >
                  + Add New User
                </button>
              )}
            ...
            <div className="space-y-6">
              ...
              <div className="portal-card rounded-2xl shadow-xl p-4 sm:p-6 md:p-8 min-w-0">
                <div className="flex flex-col gap-4 mb-6">
                  <h3 className="text-lg font-bold text-slate-900">All Expense Database</h3>
      ...
      <Modal
        ...
        title={
          selectedExpense && (
            <>
              {actionType === "approve" && <span className="text-emerald-600">✅ Approve Expense</span>}
              {actionType === "reject" && <span className="text-rose-600">❌ Reject Expense</span>}
              ...
              <form onSubmit={handleActionSubmit} className="space-y-4">
                ...
                <div className="flex justify-end gap-2 pt-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2 rounded-lg bg-[var(--af-navy)] hover:bg-[var(--af-navy-soft)] text-xs font-bold text-white shadow-lg shadow-[var(--af-navy)]/10 transition-colors cursor-pointer disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg text-xs font-bold text-white shadow-lg transition-colors cursor-pointer disabled:opacity-50 ${
                      actionType.includes("reject")
                        ...
                        : "bg-[var(--af-navy)] hover:bg-[var(--af-navy-soft)] shadow-[var(--af-navy)]/10"
                    }`}
                  >
      ...
      {activeSection === "user-management" && currentUser?.role === "ADMIN" && (
        <div className="mt-2">
          ...
          {userActionMsg && (
            <div className={`mb-4 rounded-lg px-4 py-3 text-sm font-medium ${userActionMsg.startsWith("✅") ? "bg-emerald-50 border border-emerald-200 text-emerald-700" : "bg-rose-50 border border-rose-200 text-rose-700"}`}>
              {userActionMsg}
            </div>
          )}
              ...
              <div className="bg-white border border-slate-400 rounded-2xl p-6 max-w-sm w-full mx-4 shadow-2xl">
                ...
                <div className="flex gap-3 mt-6">
                  <button onClick={() => setDeleteUserId(null)}
                    className="flex-1 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-sm font-semibold text-slate-600 transition-colors">
                    Cancel
                  </button>
